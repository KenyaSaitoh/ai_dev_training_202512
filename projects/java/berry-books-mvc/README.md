...
- **トップページ**: http://localhost:8080/berry-books-mvc
...
```bash
./gradlew :projects:java:berry-books-mvc:test
```
...
./gradlew :projects:java:berry-books-mvc:test --tests "*OrderServiceTest"

# BookServiceのテストのみを実行
./gradlew :projects:java:berry-books-mvc:test --tests "*BookServiceTest"
```

#### テストの継続的実行（変更検知）

```bash
./gradlew :projects:java:berry-books-mvc:test --continuous
```
...
```
projects/java/berry-books-mvc/build/reports/tests/test/index.html
```
...
```bash
# テストカバレッジレポートを生成
./gradlew :projects:java:berry-books-mvc:jacocoTestReport

# カバレッジレポートの場所
# projects/java/berry-books-mvc/build/reports/jacoco/test/html/index.html
```
...
```
projects/berry-books-mvc/
└── src/
    └── test/
        └── java/
            └── pro/kensait/berrybooks/service/
                ├── order/
...
```bash
# テストをクリーンビルドして実行
./gradlew :projects:java:berry-books-mvc:clean :projects:java:berry-books-mvc:test

# テスト結果をコンソールに詳細表示
./gradlew :projects:java:berry-books-mvc:test --info

# 失敗したテストのみ再実行
./gradlew :projects:java:berry-books-mvc:test --rerun-tasks

# テストとカバレッジレポートを一括生成
./gradlew :projects:java:berry-books-mvc:test :projects:java:berry-books-mvc:jacocoTestReport
```
...
```bash
# ビルドとテストを一括実行
./gradlew :projects:java:berry-books-mvc:build

# テスト失敗時に処理を停止
./gradlew :projects:java:berry-books-mvc:test --fail-fast
```
...
CDI Service (@ApplicationScoped)
    ↓
REST Client (JAX-RS Client) / DAO (@ApplicationScoped)
    ↓ HTTP / JPA
berry-books-rest API / Database (HSQLDB)
```

**注:** 顧客情報は`berry-books-rest` API経由でアクセス（外部システム連携）
