...
- ファイルが存在しない場合は`no-image.jpg`を表示
...
| 要素 | 表示/非表示 | 設計意図 |
|------|-----------|---------|
| 配送料金 | 自動表示 | 画面表示時に自動計算される |
| 配送料金計算ボタン | 非表示 | 手動操作不要（UX簡素化） |

**設計のポイント:**
- 初期表示時: ログインユーザーの住所で配送料金を自動計算
- 注文確定時: 変更された配送先住所で配送料金を自動再計算
- ユーザー操作を最小限にし、スムーズな購入体験を提供
...
| 決済方法 | 非表示 | 画面表示は不要 |
| 注文明細 | 表示 | 購入した書籍の詳細 |
| 料金情報 | 表示 | 商品合計・配送料・総合計 |

**設計のポイント:**
- 決済方法はデータとして保存されるが、画面表示は不要
- ユーザーにとって重要な情報（配送先、注文内容、料金）のみ表示
