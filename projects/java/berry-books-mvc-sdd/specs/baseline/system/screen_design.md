...
- **スタイリング**: CSS変数とクラスベースのスタイル定義
  - メインスタイルシート: `webapp/resources/css/style.css`
  - CSS変数（カラー、フォント、スペーシング等）による一元管理
  - 主要CSSクラス:
    - `.book-table` - 書籍一覧テーブル
    - `.book-thumbnail` - 書籍カバー画像サムネイル（height: 5cm）
    - `.book-image-cell` - 画像セル（中央配置）
    - `.cart-button` - カート追加ボタン
    - `.search-button` - 検索ボタン
    - `.out-of-stock` - 入荷待ち表示
    - `.error` - エラーメッセージ
    - `.header-nav`, `.header-left`, `.header-right` - ヘッダーナビゲーション（flexbox配置）
      - `.header-nav a` - ナビゲーションリンクの基本スタイル（color: `var(--primary-color)`、白背景で視認可能）
    - `.links` - ナビゲーションリンク（プライマリカラー、ホバー時にtransform効果）
    - `.search-link` - 検索ページへのリンク（プライマリカラー、ホバー時に濃色）
    - `form.login`, `form.search`, `form.customer`, `form.book` - フォームスタイル
- **カラースキーム**: 
  - プライマリ: `#CF3F4E`（ベリーレッド）
  - ホバー: `#B83542`, `#A32D3A`
  - 背景: `#ffffff`（メイン）、`#F5F5F5`（セカンダリ）
  - テキスト: `#333333`（プライマリ）、`#666666`（セカンダリ）
