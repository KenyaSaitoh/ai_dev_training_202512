...
6. アーキテクチャと設計ドキュメントを参照してください：
   - **architecture_design.md** が提供するもの：
     - **1. 技術スタック**: 
       - 1.1 コアプラットフォーム（Java, Jakarta EE, アプリサーバー、データベース、ビルドツール）
       - 1.2 Jakarta EE仕様（Faces, Persistence, Transactions, CDI等のバージョン）
       - 1.3 追加ライブラリ（ログ、テスト、REST Client等）
       - 1.4 外部API連携
     - **2. アーキテクチャ設計**:
       - 2.1 レイヤードアーキテクチャ（各レイヤーの責務と禁止事項）
       - 2.2 コンポーネントの責務
     - **3. デザインパターン**: 適用するパターン（MVC, サービスレイヤー, リポジトリ等）
     - **4. パッケージ構造と命名規則**: パッケージ編成、コンポーネント命名
     - **5. 状態管理**: CDIスコープ（@RequestScoped, @ViewScoped, @SessionScoped等）
     - **6. トランザクション管理**: トランザクション境界と戦略
     - **7. 並行制御**: 楽観的ロック戦略
     - **8. エラーハンドリング方針**: 例外階層とハンドリングフロー
     - **9. セキュリティ**: 認証フロー、セキュリティ対策
     - **10. データベース構成**: 永続化構成、コネクションプール
     - **11. ログ戦略**: ログフレームワーク、ログレベル、出力方針
     - **12. ビルド＆デプロイ**: ビルドプロセス、Gradleタスク
     - **13. テスト戦略**: テストピラミッド、カバレッジ目標、テスト方針
     - **14. パフォーマンス考慮事項**: 最適化戦略、期待パフォーマンス
     - **16. 開発ガイドライン**: コード標準
