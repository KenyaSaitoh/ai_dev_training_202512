...
./gradlew :projects:java:berry-books-mvc-vibe:setupHsqldb

# 2. プロジェクトをビルド
./gradlew :projects:java:berry-books-mvc-vibe:war

# 3. プロジェクトをデプロイ
./gradlew :projects:java:berry-books-mvc-vibe:deploy
```
...
```bash
# プロジェクトをアンデプロイ
./gradlew :projects:java:berry-books-mvc-vibe:undeploy
```
...
./gradlew exploreExcelFiles -PtargetDir=projects/java/berry-books-mvc-vibe/spec
```
...
```
projects/java/berry-books-mvc-vibe/spec/
...
- **トップページ**: http://localhost:8080/berry-books-mvc-vibe
...
```bash
./gradlew :projects:java:berry-books-mvc-vibe:test
```
...
./gradlew :projects:java:berry-books-mvc-vibe:test --tests "*OrderServiceTest"

# BookServiceのテストのみを実行
./gradlew :projects:java:berry-books-mvc-vibe:test --tests "*BookServiceTest"
```

#### テストの継続的実行（変更検知）

```bash
./gradlew :projects:java:berry-books-mvc-vibe:test --continuous
```
...
```
projects/java/berry-books-mvc-vibe/build/reports/tests/test/index.html
```
...
```bash
# テストカバレッジレポートを生成
./gradlew :projects:java:berry-books-mvc-vibe:jacocoTestReport

# カバレッジレポートの場所
# projects/java/berry-books-mvc-vibe/build/reports/jacoco/test/html/index.html
```
...
```bash
# テストをクリーンビルドして実行
./gradlew :projects:java:berry-books-mvc-vibe:clean :projects:java:berry-books-mvc-vibe:test

# テスト結果をコンソールに詳細表示
./gradlew :projects:java:berry-books-mvc-vibe:test --info

# 失敗したテストのみ再実行
./gradlew :projects:java:berry-books-mvc-vibe:test --rerun-tasks

# テストとカバレッジレポートを一括生成
./gradlew :projects:java:berry-books-mvc-vibe:test :projects:java:berry-books-mvc-vibe:jacocoTestReport
```
...
```bash
# ビルドとテストを一括実行
./gradlew :projects:java:berry-books-mvc-vibe:build

# テスト失敗時に処理を停止
./gradlew :projects:java:berry-books-mvc-vibe:test --fail-fast
```
...
> **Note:** これらのプロジェクトは**このプロジェクト（berry-books-mvc-vibe）でセットアップしたデータベース**を使用します。
